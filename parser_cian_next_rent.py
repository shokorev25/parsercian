...
# Список ID всех нужных городов Иркутской области
REGION_IDS = [
...
def get_json(region_id, page=1):
    ...
    payload = {
        "jsonQuery": {
            ...
            "offer_type": {"type": "terms", "value": ["rent"]},
...
def main():
    ...
    if all_offers:
        with open("irkutsk_region_offices_rent.csv", "w", encoding="utf-8", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=all_offers[0].keys())
            writer.writeheader()
            writer.writerows(all_offers)
        print("Данные сохранены в irkutsk_region_offices_rent.csv")
    else:
